    ...
    def sync_chain(self):
        ...
        for peer in PEERS:
            try:
                ...
                if len(temp_chain) > len(longest) and self.valid_chain(temp_chain):
                    longest = temp_chain
...
def home():
    return {
        "status": "DYNAX BITCOIN NETWORK LAYER ACTIVE",
...
@app.route("/sync")
def sync():
    result = node.sync_chain()
    return {"synced": result, "blocks": len(node.chain)}


if __name__ == "__main__":
    print("=== DYNAX BITCOIN NETWORK LAYER STARTED ===")
